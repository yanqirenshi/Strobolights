(in-package :cl-user)
(defpackage strobolights.config
            (:use :cl)
            ...
            (:export #:config
                     #:print-config
                     #:*application-root*))
(in-package :strobolights.config)

(defparameter *application-root* (asdf:system-source-directory :strobolights))

(defvar *tree-stor* (merge-pathnames #P"src/data/" *application-root*))
(defvar *tree* (make-tree (ensure-directories-exist *tree-stor*)))

(defvar *application-code* :strobolights)
